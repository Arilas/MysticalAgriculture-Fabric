        ...
        if (attacker instanceof Player player) {
            if (!player.getCooldowns().isOnCooldown(stack)) {
                ...
                for (var aoeEntity : entities) {
                    if (aoeEntity != player && aoeEntity != target && !player.isAlliedTo(aoeEntity)) {
                        aoeEntity.knockback(0.4F, Mth.sin(player.getYRot() * 0.017453292F), -Mth.cos(player.getYRot() * 0.017453292F), source, 1.0F);
                        aoeEntity.hurt(source, 5.0F + (5.0F * this.amplifier));
                    }
                }
