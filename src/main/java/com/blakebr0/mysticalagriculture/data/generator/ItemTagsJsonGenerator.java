...
public class ItemTagsJsonGenerator extends TagsProvider<Item> {
    ...
    protected void addTags(HolderLookup.Provider provider) {
        for (var crop : CropRegistry.getInstance().getCrops()) {
            this.tag(MysticalAgricultureTags.Items.ESSENCES)
                    .add(crop.getEssenceItem().builtInRegistryHolder().key());
            this.tag(MysticalAgricultureTags.Items.SEEDS)
                    .add(crop.getSeedsItem().builtInRegistryHolder().key());
        }
    }
}
